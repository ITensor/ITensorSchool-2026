...
- `square_energies::Vector{Float64}`: Vector of mean square energies from each METTS sample.
...
function main(;
    ...
    for step in 1:(Nwarm + NMETTS)
        ...
        if step > Nwarm
            ...
            push!(square_energies, inner(H, psi, H, psi))
            if outputlevel > 0 && step % print_every == 0
    ...
    return (;
        H, psi, betas, energies, square_energies, energy_dmrg, nsite, beta, betastep, cutoff,
        NMETTS, Nwarm,
    )
end

"""
    specific_heat(res::NamedTuple)

Estimate the specific heat per site from the METTS ensemble averages of the energy and the
mean square energy. `res` is expected to be a `NamedTuple` with fields `beta`, `nsite`,
`energies`, and `square_energies`, such as the results of `main`.
"""
function specific_heat(res::NamedTuple)
    (; beta, nsite, energies, square_energies) = res
    return beta^2 * (mean(square_energies) - mean(energies)^2) / nsite
end
